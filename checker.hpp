...
private:
    ...
    char* m_addr;
public:
    explicit urlChecker(char* address): m_addr{address}{
        WSADATA wsa;
    ...
    //Connect to target addr, resolves address before connect. eg: localhost -> 127.0.0.1
    ...
    void CheckCode200(const int &buffsize) {
...
public:
    explicit Application(int &argc, char* argv[]): m_argc{argc}, m_argv{argv}{
        if (argc < 2){
    ...
    void run(){
        ...
        std::cout << "Duration to send " << m_argc - 1 << " request: " << duration.count() << "ms\n";
    }
};
